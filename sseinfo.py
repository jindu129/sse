    ...
    for url in urls:
        try:
            ...
            if resp.headers['content-type'] == 'application/pdf':
                ...
                if resp.ok:
                    with open(os.path.basename(url), 'wb') as outfile:  # 表示以二进制写方式打开,只能写文件, 如果文件不存在,创建该文件;如果文件已存在,则覆盖写。
                        outfile.write(resp.content)
                        print("Saved {} to file {}".format(url, os.path.basename(url)))
                else:
                    print('GET request for URL {} failed with HTTP status "{} {}"'.format(url, resp.status_code,
                                                                                          resp.reason))
